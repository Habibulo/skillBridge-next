...
import { REACT_APP_API_URL, jobRank } from '../../config';
...
const AddJob = ({ initialValues, ...props }: any) => {
	...
	useEffect(() => {
		setinsertJobData({
			...
			jobEmploymentMode: getJobData?.getJob ? getJobData?.getJob?.jobEmploymentMode : '',
			jobLocation: getJobData?.getJob ? getJobData?.getJob?.jobLocation : '',
			jobAddress: getJobData?.getJob ? getJobData?.getJob?.jobAddress : '',
			jobRemoteAvailable: getJobData?.getJob ? getJobData?.getJob?.jobRemoteAvailable : false,
			jobNegotiableSalary: getJobData?.getJob ? getJobData?.getJob?.jobNegotiableSalary : false,
			jobExperienceRequired: getJobData?.getJob ? getJobData?.getJob?.jobExperienceRequired : 0,
			jobPositionsAvailable: getJobData?.getJob ? getJobData?.getJob?.jobPositionsAvailable : 0,
			jobRank: getJobData?.getJob ? getJobData?.getJob?.jobRank : 0,
			jobDescription: getJobData?.getJob ? getJobData?.getJob?.jobDescription : '',
	...
	const doDisabledCheck = () => {
		if (
			...
			insertJobData.jobEmploymentMode === '' || // @ts-ignore
			insertJobData.jobLocation === '' || // @ts-ignore
			insertJobData.jobAddress === '' || // @ts-ignore
			insertJobData.jobPositionsAvailable === 0 || // @ts-ignore
			insertJobData.jobRemoteAvailable === false ||
			insertJobData.jobNegotiableSalary === false ||
			insertJobData.jobRank === 0 ||
			insertJobData.jobExperienceRequired === 0 ||
			insertJobData.jobDescription === '' ||
	...
	const insertjobHandler = useCallback(async () => {
		try {
			...
			await sweetMixinSuccessAlert('This job has been created successfully.');
			await router.push({
				pathname: '/mypage',
				query: {
					category: 'myJobs',
	...
	const updatejobHandler = useCallback(async () => {
		try {
			...
			await sweetMixinSuccessAlert('This job has been update successfully.');
			await router.push({
				pathname: '/mypage',
				query: {
						...
						<Stack className="description-box">
							<Stack className="config-column">
								...
								<input
									...
									onChange={({ target: { value } }) =>
										setinsertJobData({ ...insertJobData, jobTitle: value })
									}
								/>
							</Stack>
							...
							<Stack className="config-row">
								...
								<Stack className="price-year-after-price">
									...
									<select
										...
										defaultValue={insertJobData.jobEmploymentMode || 'select'}
										value={insertJobData.jobEmploymentMode || 'select'}
										onChange={({ target: { value } }) =>
											// @ts-ignore
									...
									>
										<>
											...
											{jobEmploymentMode.map((type: any) => (
							...
							<Stack className="config-row">
								...
								<Stack className="price-year-after-price">
									...
									<input
										...
										onChange={({ target: { value } }) =>
											setinsertJobData({ ...insertJobData, jobAddress: value })
										}
									/>
								</Stack>
							</Stack>
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertJobData.jobRemoteAvailable ? 'yes' : 'no'}
										defaultValue={insertJobData.jobRemoteAvailable ? 'yes' : 'no'}
										onChange={({ target: { value } }) =>
											setinsertJobData({ ...insertJobData, jobRemoteAvailable: value === 'yes' })
										}
									>
								...
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertJobData.jobNegotiableSalary ? 'yes' : 'no'}
										defaultValue={insertJobData.jobNegotiableSalary ? 'yes' : 'no'}
										onChange={({ target: { value } }) =>
											setinsertJobData({ ...insertJobData, jobNegotiableSalary: value === 'yes' })
										}
									>
										<option disabled={true} selected={true}>
											Select
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertJobData.jobExperienceRequired || 'select'}
										defaultValue={insertJobData.jobExperienceRequired || 'select'}
										onChange={({ target: { value } }) =>
											setinsertJobData({ ...insertJobData, jobExperienceRequired: parseInt(value) })
										}
									>
								...
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertJobData.jobPositionsAvailable || 'select'}
										defaultValue={insertJobData.jobPositionsAvailable || 'select'}
										onChange={({ target: { value } }) =>
											setinsertJobData({ ...insertJobData, jobPositionsAvailable: parseInt(value) })
										}
									>
										<option disabled={true} selected={true} value={'select'}>
											Select
								...
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertJobData.jobRank || 'select'}
										defaultValue={insertJobData.jobRank || 'select'}
										onChange={({ target: { value } }) =>
											setinsertJobData({ ...insertJobData, jobRank: parseInt(value) })
										}
									>
										<option disabled={true} selected={true} value={'select'}>
											Select
										</option>
										{jobRank.map((square: number) => {
											if (square !== 0) {
							...
							<Stack className="config-column">
								...
								<textarea
									...
									value={insertJobData.jobDescription}
									onChange={({ target: { value } }) =>
										setinsertJobData({ ...insertJobData, jobDescription: value })
									}
								></textarea>
							</Stack>
						</Stack>
...
AddJob.defaultProps = {
	initialValues: {
		...
		jobLocation: '',
		jobAddress: '',
		jobRemoteAvailable: false,
		jobNegotiableSalary: false,
